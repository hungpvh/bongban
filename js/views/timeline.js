...
export function renderTimeline() {
    ...
    if (!game.danh_sach_diem || game.danh_sach_diem.length === 0) {
        ...
        filteredPoints.forEach((pt, filteredIdx) => {
            ...
            const isExpanded = state.timelineExpandedIdx === idx;
            let cardContentHtml = '';
                ...
                cardContentHtml = `
                    <div class="flex flex-wrap items-start justify-between gap-2 mb-1">
                        ...
                        <div class="flex gap-1 shrink-0 ml-1">
                            <button onclick="event.stopPropagation(); window.app.actions.rally.editRally(${idx}); window.app.navigate('rallyEntry', {selectedMatchId: '${match.id_tran_dau}', selectedGameId: '${state.selectedGameId}'})" class="p-2 sm:p-2.5 -m-1 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded transition" title="Sửa" aria-label="Sửa">
                                <i data-lucide="pencil" class="w-4 h-4"></i>
                            </button>
                            <button onclick="event.stopPropagation(); window.app.actions.rally.deleteRally(${idx})" class="p-2 sm:p-2.5 -m-1 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded transition" title="Xóa" aria-label="Xóa">
